...
import { DanmuSession, TokenInfo } from '../types';
import { AcFunDanmu } from '../proto/acfun';
import Long from 'long';  // 导入Long类型处理大整数
...
export function encode(
  ...
  payload: AcFunDanmu.Im.Basic.IUpstreamPayload,
  encryptionKey: Buffer
): Buffer {
  // 1. 序列化 PacketHeader
  const headerBytes = AcFunDanmu.Im.Basic.PacketHeader.encode(header).finish();
  const headerLength = headerBytes.length;
  
  // 2. 序列化 UpstreamPayload
  const payloadBytes = AcFunDanmu.Im.Basic.UpstreamPayload.encode(payload).finish();
...
export function decode(
  ...
  securityKey: Buffer,
  sessionKey?: Buffer
): { header: AcFunDanmu.Im.Basic.IPacketHeader; payload: AcFunDanmu.Im.Basic.IDownstreamPayload } | null {
  try {
    let offset = 0;
    ...
    const header = AcFunDanmu.Im.Basic.PacketHeader.decode(headerBytes);
    offset += headerLength;
    ...
    if (header.encryptionMode === AcFunDanmu.Im.Basic.PacketHeader.EncryptionMode.kEncryptionNone) {
      decryptionKey = null;
    } else if (header.encryptionMode === AcFunDanmu.Im.Basic.PacketHeader.EncryptionMode.kEncryptionServiceToken) {
      decryptionKey = securityKey;
    } else if (header.encryptionMode === AcFunDanmu.Im.Basic.PacketHeader.EncryptionMode.kEncryptionSessionKey) {
    ...
    const payload = AcFunDanmu.Im.Basic.DownstreamPayload.decode(payloadBytes);
 ...
 */
export function buildRegisterRequest(tokenInfo: TokenInfo): AcFunDanmu.Im.Basic.IRegisterRequest {
  return {
    appInfo: {
    ...
    deviceInfo: {
      platformType: AcFunDanmu.Im.Basic.DeviceInfo.PlatformType.H5_WINDOWS,
      deviceModel: 'h5'
    },
    presenceStatus: AcFunDanmu.Im.Basic.RegisterRequest.PresenceStatus.kPresenceOnline,
    appActiveStatus: AcFunDanmu.Im.Basic.RegisterRequest.ActiveStatus.kAppInForeground,
    instanceId: 0,
    ztCommonInfo: {
      kpn: 'ACFUN_APP',
      kpf: 'PC_WEB',
 ...
 */
export function buildKeepAliveRequest(): AcFunDanmu.Im.Basic.IKeepAliveRequest {
  return {
    presenceStatus: AcFunDanmu.Im.Basic.RegisterRequest.PresenceStatus.kPresenceOnline,
    appActiveStatus: AcFunDanmu.Im.Basic.RegisterRequest.ActiveStatus.kAppInForeground
  };
 ...
 */
export function buildEnterRoomRequest(
  enterRoomAttach: string
): AcFunDanmu.IZtLiveCsEnterRoom {
  return {
    enterRoomAttach,
 ...
 * 构造 UserExit 消息
 */
export function buildUserExitRequest(): AcFunDanmu.IZtLiveCsUserExit {
  return {};
}
 ...
 */
export function buildUnregisterRequest(): AcFunDanmu.Im.Basic.IUnregisterRequest {
  return {};
}
 ...
 * 构造 ZtLiveCsCmd 封装
 */
export function buildZtLiveCsCmd(
  cmdType: string,
  payload: Uint8Array,
  ticket: string,
  liveId: string
): AcFunDanmu.IZtLiveCsCmd {
  return {
    cmdType,
    payload,
    ticket,
...
export function sendCommand(
  ...
  seqId: number,
  instanceId: number | Long,  // 支持Long类型，避免精度丢失
  uid: string,
  encryptionKey: Buffer,
  encryptionMode: AcFunDanmu.Im.Basic.PacketHeader.EncryptionMode,
  appId: number = 0  // 默认为0，如果会话中有appID则使用那个值
): Buffer {
  // 构造 UpstreamPayload
