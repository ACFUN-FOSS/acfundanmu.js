...
import { ApiResponse, Manager, KickRecord } from '../types';
...
export class ManagerService {
   ...
   * 获取房管列表
   */
  public async getManagerList(): Promise<ApiResponse<any>> {
    try {
      ...
      const managerList = list.map((item: any) => ({
   ...
   * 添加房管
   */
  public async addManager(managerUID: number): Promise<ApiResponse<void>> {
    try {
      // 从HTTP客户端的认证头中获取并验证token信息
      const { tokenInfo, error } = this.httpClient.getValidatedTokenInfo();
      if (error || !tokenInfo) {
   ...
   */
  public async deleteManager(managerUID: number): Promise<ApiResponse<void>> {
    try {
      // 从HTTP客户端的认证头中获取并验证token信息
      const { tokenInfo, error } = this.httpClient.getValidatedTokenInfo();
      if (error || !tokenInfo) {
   ...
   * 获取主播踢人记录
   */
  public async getAuthorKickRecords(liveId: string, count: number = 20, page: number = 0): Promise<ApiResponse<KickRecord[]>> {
    try {
      // 从HTTP客户端的认证头中获取并验证token信息
      const { tokenInfo, error } = this.httpClient.getValidatedTokenInfo();
      if (error || !tokenInfo) {
      ...
      // 解析踢人记录数据
      const list = data.data?.list || [];
      const kickRecords = list.map((item: any) => ({
        userId: item.userId?.toString() || '',
        nickname: item.nickname || '',
   ...
   * 房管踢人
   */
  public async managerKick(liveID: string, kickedUID: number): Promise<ApiResponse<void>> {
    try {
      // 从HTTP客户端的认证头中获取并验证token信息
      const { tokenInfo, error } = this.httpClient.getValidatedTokenInfo();
      if (error || !tokenInfo) {
   ...
   * 主播踢人
   */
  public async authorKick(liveID: string, kickedUID: number): Promise<ApiResponse<void>> {
    try {
      // 从HTTP客户端的认证头中获取并验证token信息
      const { tokenInfo, error } = this.httpClient.getValidatedTokenInfo();
      if (error || !tokenInfo) {
