...
export class GiftService {
   ...
   * 获取全部礼物列表
   */
  public async getAllGiftList(): Promise<ApiResponse<Array<{
    giftID: number;
    giftName: string;
    arLiveName: string;
    ...
    try {
      ...
      const gifts = giftList.map((gift: any) => ({
   ...
   * 获取直播间礼物列表
   */
  public async getLiveGiftList(liveID: string): Promise<ApiResponse<Array<{
    giftID: number;
    giftName: string;
    arLiveName: string;
    payWalletType: number;
    ...
    try {
      ...
      // 发送POST请求
      const response = await this.httpClient.post<any>(url, formData.toString(), { headers });
