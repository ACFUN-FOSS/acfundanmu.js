...
class LiveService {
     ...
     * 获取直播推流地址
     */
    async getStreamUrl(liveId) {
        // 使用核心组件进行快手API调用
        const response = await (0, ApiUtils_1.kuaiShouApiPost)(this.httpClient, 'https://api.kuaishouzt.com/rest/zt/live/web/obs/config?subBiz=mainApp&kpn=ACFUN_APP&kpf=PC_WEB', '获取推流地址', { liveId });
        if (!response.success) {
            return response;
        }
     ...
     * 获取热门直播列表
     */
    async getHotLives(category, page, size) {
        try {
            // 设置默认参数
            ...
            // 设置请求头 - 使用核心组件构建通用请求头
            const headers = (0, ApiUtils_1.buildCommonHeaders)();
            headers['Referer'] = 'https://live.acfun.cn/';
            // 使用apiGet发送请求
            const response = await (0, ApiUtils_1.apiGet)(this.httpClient, url, '获取热门直播列表');
            if (!response.success) {
                return response;
            }
    ...
    async getLiveCategories() {
        try {
            ...
            if (data.typeList && Array.isArray(data.typeList)) {
                // 转换数据格式以匹配测试用例期望的结构
                const convertedData = data.typeList.map((item) => ({
     ...
     * 获取直播统计数据
     */
    async getLiveStatistics(liveId) {
        try {
            // 首先通过liveId获取主播的用户ID
            ...
            // 在直播列表中查找对应的liveId
            const targetLive = hotLivesResponse.data?.lives?.find((live) => live.liveId === liveId);
            if (!targetLive) {
                return {
                    success: false,
     ...
     * 获取直播列表
     */
    async getLiveList(page = 1, pageSize = 20) {
        try {
            // 设置默认参数 - 使用A站API格式
     ...
     * 获取直播统计数据
     */
    async getLiveStatisticsByDays(days) {
        try {
            // 验证days参数
            if (days < 1) {
                return {
            ...
            if (data.liveDetail && typeof data.liveDetail === 'object') {
                Object.keys(data.liveDetail).forEach(date => {
                    ...
                    if (Array.isArray(detailArray)) {
                        liveStatistics.liveDetail[date] = detailArray.map((detail) => ({
            ...
            if (Array.isArray(data.dailyData)) {
                liveStatistics.dailyData = data.dailyData.map((daily) => ({
                    date: daily.date || '',
     ...
     * 获取转码信息
     */
    async getTranscodeInfo(streamName) {
        try {
            ...
            // 转换数据格式以匹配函数签名
            const transcodeInfos = transcodeInfoList.map((info) => ({
                streamURL: {
                    url: info.pullUrl || '',
     ...
     * 开始直播
     */
    async startLiveStream(title, coverFile, streamName, portrait, panoramic, categoryID, subCategoryID) {
        try {
            // 从HTTP客户端的认证头中获取并验证token信息
            const { tokenInfo, error: tokenError } = this.httpClient.getValidatedTokenInfo();
            if (tokenError || !tokenInfo) {
            ...
            const cookieHeader = (0, ApiUtils_1.buildCookieString)(tokenInfo.cookies, tokenInfo.deviceID);
            // 使用apiPost发送请求
            const response = await (0, ApiUtils_1.apiPost)(this.httpClient, fullUrl, requestBody, {
                headers: {
     ...
     * 更改直播间标题和封面
     */
    async updateLiveRoom(title, coverFile, liveId) {
        try {
            // 从HTTP客户端的认证头中获取并验证token信息
            const { tokenInfo, error } = this.httpClient.getValidatedTokenInfo();
            if (error || !tokenInfo) {
     ...
     * 设置是否允许观众剪辑直播录像
     */
    async setLiveClipPermission(canCut) {
        try {
            // 从HTTP客户端的认证头中获取并验证token信息
            const { tokenInfo, error: tokenError } = this.httpClient.getValidatedTokenInfo();
            if (tokenError || !tokenInfo) {
     ...
     * 获取直播剪辑信息
     */
    async getLiveClipInfo(liverUID, liveID) {
        try {
            // 从HTTP客户端的认证头中获取并验证token信息
            const { tokenInfo, error: tokenError } = this.httpClient.getValidatedTokenInfo();
            if (tokenError || !tokenInfo) {
    ...
    async getSummary(liveId) {
        try {
            ...
            const response = await (0, ApiUtils_1.kuaiShouApiPost)(this.httpClient, 'https://api.kuaishouzt.com/rest/zt/live/web/endSummary?subBiz=mainApp&kpn=ACFUN_APP&kpf=PC_WEB', '获取直播总结', { liveId });
            if (!response.success) {
                return {
                    success: false,
