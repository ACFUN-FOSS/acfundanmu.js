...
class AuthService {
    constructor(httpClient) {
        this.qrLoginToken = null;
        this.qrLoginSignature = null;
    ...
    async qrLogin() {
        try {
            ...
            // 使用无需认证的GET请求函数
            const response = await (0, ApiUtils_1.apiGetWithoutAuth)(this.httpClient, url, '获取登录二维码');
    ...
    async checkQrLoginStatus() {
        ...
        try {
            ...
            // 使用无需认证的GET请求函数
            const response = await (0, ApiUtils_1.apiGetWithoutAuth)(this.httpClient, url, '检查二维码登录状态');
            if (!response.success) {
                return response;
            }
            const data = response.data;
    ...
    async confirmQrLogin() {
        ...
        try {
            ...
            // 使用无需认证的GET请求函数
            const response = await (0, ApiUtils_1.apiGetWithoutAuth)(this.httpClient, url, '确认二维码登录');
            if (!response.success) {
                return response;
            }
            const data = response.data;
    ...
    async getServiceToken(cookies) {
        try {
            ...
            // 使用无需认证的POST请求函数
            const response = await (0, ApiUtils_1.apiPostWithoutAuth)(this.httpClient, url, '获取服务令牌', formData.toString(), {
                headers: {
